...
{
    public partial class InventoryChangeLot : PageCustom
    {
        #region ViewState
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!Page.IsPostBack)
                {
                    ddlReasonCode.MethodQueryProperty = delegate () { return Access.MasterData.Reason.Instance.GetQuery("ChangeLot"); };
                    ddlReasonCode.BindDataSource();

                    hid_control_lot.SetValue(true);
                }
            }
            catch (Exception ex)
            {
        ...
        protected void btnChangeLot_Click(object sender, EventArgs e)
        {
            try
            {
        ...
        protected void btnConfirmChange_Click(object sender, EventArgs e)
        {
            try
            {
                using (var acc = new Access.Transaction.Inventory.InventoryChange())
                {
                    ...
                    string lot_number = txtLot.GetValue();
                    double? qty = txtQty.GetValue();
                    Guid reason_id = ddlReasonCode.GetValue();
                    string remark = txtRemark.GetValue();

                    bool isSuccess = acc.Change_Lot(listKey, lot_number, qty, reason_id, remark);
                    if (isSuccess)
                    {

...
{
    public class InboundCancelPallet : AGridObjectSourceQuery, IDisposable
    {
        public WMSEntities _Model { get; set; }

        public InboundCancelPallet()
        {
        ...
        public static InboundCancelPallet Instance
        {
            get
            {
                using (InboundCancelPallet _Instance = new InboundCancelPallet())
                {
                    return _Instance;
                }
            }
        ...
        public override IQueryable<dynamic> InitialQueryView()
        {
            ...
            var prin_label = _Model.t_wms_print_label.Where(w => w.is_received == "NO").Select(s => s.inbound_order_master_id).Distinct().ToList();
            
            query = from master in query
                    where prin_label.Contains(master.KeyId)
                    select master;
    ...
    public class InboundCancelPalletDetail : AEntityFormCommand<t_wms_print_label>
    {
        public WMSEntities _Model { get; set; }

        public InboundCancelPalletDetail()
        {
        ...
        public override IQueryable<dynamic> InitialQueryView()
        {
            ...
            var result = from rows in _Model.t_wms_print_label
                         where rows.inbound_order_master_id == inbound_order_master_id && rows.is_received == "NO"
                         select new
                         {

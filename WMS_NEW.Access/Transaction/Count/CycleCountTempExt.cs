...
namespace WMS_NEW.Access.Transaction.Count
{
    ...
    public class CycleCountTempExt : AGridObjectSourceQuery, IDisposable
    {
        public WMSEntities _Model { get; set; }

        public CycleCountTempExt()
        {
        ...
        public static CycleCountTempExt Instance
        {
            get
            {
                using (CycleCountTempExt _Instance = new CycleCountTempExt())
                {
                    return _Instance;
                }
            }
        ...
        {
            var temp = from rows in this._Model.t_wms_count_temporary_external
            ...
            return this._Model.Save(this, delegate ()
            {
                ...
                t_wms_count_temporary_external ent;
                foreach (var item in items)
                {
                    ent = new t_wms_count_temporary_external();
                    ...
                    this._Model.t_wms_count_temporary_external.Add(ent);
                }
        ...
        public bool AddTempByInventory(FilterList _filters, Guid _wh_master_id, Guid _owner_id, Guid _customer_id)
        {
            return this._Model.GetDataBy(this, delegate ()
            {

                var rules = from rows in this._Model.t_wms_rule
                            where rows.is_active == "YES" && rows.rule_code == "RULE_LOCATION_TYPE_FOR_COUNT_EXTERNAL"
                            select rows.value;

                var result = from rows in this._Model.v_wms_inventory_data_by_serial
                             join cus_loc in _Model.t_wms_customer_location on rows.location_id equals cus_loc.location_id
                             where rows.wh_master_id == _wh_master_id && rows.owner_id == _owner_id && rules.Contains(rows.loc_type) && cus_loc.customer_id == _customer_id
                             select new
                             {
                                 rows.item_number,
                                 rows.category_id,
        ...
        public int CountTempByUser(string _userId)
        {
            return this._Model.GetDataBy(this, delegate ()
            {
                return this._Model.t_wms_count_temporary_external.Count(qry => qry.create_by == _userId);
            });
        }

        public void DeleteTempByUser(string _userId)
        {
            this._Model.usp_com_delete_count_temp_external_byuser(_userId);
        }
        public bool DeleteByDefault(Guid location_id, Guid wh_item_master_id)
        {
            return this._Model.Update(this, delegate ()
            {
                var result = this._Model.t_wms_count_temporary_external.Where(qry => qry.location_id == location_id && qry.wh_item_master_id == wh_item_master_id);
                foreach (var item in result)
                {
                    this._Model.t_wms_count_temporary_external.Remove(item);
                }
        ...
        public dynamic GetDeleteDtoById(Guid _id)
        {
            return this._Model.GetDataBy(this, delegate ()
            {
                var result = (from rows in this._Model.t_wms_count_temporary_external
